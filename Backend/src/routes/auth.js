...
const schema_1 = require("../models/schema"); // Import the updated User model
const hash_1 = require("../utils/hash");
const otp_1 = require("../utils/otp");
const googleapis_1 = require("googleapis");
const router = (0, express_1.Router)();
// Google Sign-In (Basic Profile and Email)
router.get("/google", passport_1.default.authenticate("google-signin", { scope: ["profile", "email"], prompt: "select_account" }));
// Google Sign-In Callback
router.get("/google/callback", passport_1.default.authenticate("google-signin", { failureRedirect: "/" }), async (req, res) => {
    try {
        const userProfile = req.user; // Keeping profile type as any
        ...
        let user = await schema_1.User.findOne({ email: userEmail });
        if (!user) {
            // If user does not exist, create a new user
            user = new schema_1.User({
                username: username,
                email: userEmail,
...
router.get("/google/drive", passport_1.default.authenticate("google-drive", { scope: ["profile", "email", "https://www.googleapis.com/auth/drive.readonly"] }));
// Google Drive Callback
router.get("/google/drive/callback", passport_1.default.authenticate("google-drive", { failureRedirect: "/" }), async (req, res) => {
    try {
        const userProfile = req.user; // Keeping profile type as any
        const accessToken = userProfile.accessToken; // Access token from Google
...
router.get("/google/drive/files", async (req, res) => {
    try {
        ...
        const syncedFiles = await schema_1.GoogleDriveFile.find({
            userId,
            fileId: { $in: fileIds }
        });
...
router.post("/login", async (req, res) => {
    ...
    try {
        let user = await schema_1.User.findOne({ email, authenticationType: "email" }).select("+password");
        ...
        if (!user) {
            ...
            await (0, otp_1.sendOTP)(user, email);
        ...
        const refreshToken = jsonwebtoken_1.default.sign({ userId: user._id }, process.env.JWT_SECRET, { expiresIn: "7d" });
        ...
        const fileList = await schema_1.UserFile.find({ userId: user._id });
...
router.post("/verify-otp", async (req, res) => {
    ...
    try {
        ...
        const user = await schema_1.User.findOne({ email, authenticationType: "email" }).select("+otp otpExpiresAt verified");
        if (!user) {
        ...
        if (user.otp === otp &&
            ...
            await user.save();
            // Generate JWT tokens
            const token = jsonwebtoken_1.default.sign({ email: user._id }, process.env.JWT_SECRET, {
                expiresIn: "1h",
            });
            const refreshToken = jsonwebtoken_1.default.sign({ email: user._id }, process.env.JWT_SECRET, { expiresIn: "7d" });
            console.log(token, refreshToken);
            // Set tokens in cookies
            res.cookie("token", token, {
                httpOnly: true,
